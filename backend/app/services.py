...
def run_all_checks(db: Session) -> int:
    monitored_urls = db.query(MonitoredURL).filter(MonitoredURL.is_active == True).all()
...
def build_url_summary(monitored_url: MonitoredURL) -> dict:
    ...
    if not monitored_url.is_active:
        status = "PAUSED"
    elif not latest:
        status = "PENDING"
    else:
...
def toggle_url_active(db: Session, url_id: int) -> MonitoredURL | None:
    monitored_url = db.query(MonitoredURL).filter(MonitoredURL.id == url_id).first()
    if not monitored_url:
        return None
